 ...
 */
export async function getPinterestCredentials() {
  let clientId = process.env.PINTEREST_APP_ID || '';
  let clientSecret = process.env.PINTEREST_APP_SECRET || '';

  // Fallback to database settings if environment variable is not defined on host
  if (!clientSecret || !clientId) {
    try {
      const settings = await getSettings();
      const pConfig = settings?.socialMedia?.pinterest || {};
      if (!clientId && pConfig.appId) clientId = pConfig.appId;
      if (!clientSecret && pConfig.appSecret) clientSecret = pConfig.appSecret;
    } catch (_) {}
  }

  if (!clientId) clientId = '1607585';
 ...
 */
export async function getPinterestAuthUrl({ redirectUri, state = 'sphinix_pinterest_oauth' }) {
  const { clientId } = await getPinterestCredentials();
  if (!clientId) {
    throw new Error('Pinterest App ID is missing. Please set PINTEREST_APP_ID in your environment.');
  }
...
export async function exchangeCodeForTokens({ code, redirectUri }) {
  ...
  if (!clientId || !clientSecret) {
    throw new Error('Pinterest App Secret is missing. Please set PINTEREST_APP_SECRET in your environment or enter it in Dashboard Settings.');
  }
