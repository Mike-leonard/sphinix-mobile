...
import { getResolvedSettings } from '@/actions/settings';

/**
 * Creates and returns a Nodemailer transporter instance using SMTP settings
 */
...
export async function sendSupportEmail({ name, email, category, subject, message }) {
  ...
  const receiverEmail = smtpConfig.receiverEmail || process.env.CONTACT_RECEIVER_EMAIL || smtpConfig.user || process.env.SMTP_USER || 'support@sphinix-mobile.com';
  const defaultFrom = `"Sphinix Mobile Support" <${smtpConfig.user || process.env.SMTP_USER || 'noreply@sphinix-mobile.com'}>`;
  const fromAddress = smtpConfig.from || process.env.SMTP_FROM || defaultFrom;
