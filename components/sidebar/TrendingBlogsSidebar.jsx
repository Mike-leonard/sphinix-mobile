...
export default async function TrendingBlogsSidebar({ limit = 4 }) {
  const latestBlogs = await publishedBlogs(limit);
      ...
      <div className="flex flex-col gap-4">
        {latestBlogs.map(blog => (
          <Link
            ...
            key={blog.id}
            style={{ fontSize: "var(--font-size-link-nav, var(--font-size-link-default))" }}
          ...
          >
            <div className="w-16 h-16 rounded-xl bg-slate-100 dark:bg-slate-950 flex-shrink-0 relative overflow-hidden flex items-center justify-center">
              <div className={`absolute inset-0 bg-gradient-to-br ${blog.gradientColor || 'from-purple-500 to-indigo-500'} opacity-20`} />
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p style={{ fontSize: "var(--font-size-p-form, var(--font-size-p-default))" }} className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
                {blog.readTime || '3 min read'} • {blog.publishedAt ? new Date(blog.publishedAt).toLocaleDateString() : (blog.date || '')}
              </p>
            </div>
          </Link>
        ))}
