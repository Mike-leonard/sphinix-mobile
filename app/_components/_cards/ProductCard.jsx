...
export default function ProductCard({ product, isComparing, onToggleCompare }) {
  return (
    <div className="group rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 p-5 space-y-4 hover:border-brand-500/40 hover:shadow-xl hover:shadow-brand-500/5 transition-all duration-300 flex flex-col justify-between">
      <div>
        ...
        <div className="relative rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-950 p-4 h-48 flex items-center justify-center group-hover:scale-[1.01] transition-transform duration-300">
          ...
          <div className="relative w-24 h-40 rounded-2xl bg-white dark:bg-slate-900 border-2 border-slate-300 dark:border-slate-700 shadow-md p-1.5 flex flex-col">
            ...
            <div className={`flex-1 rounded-xl bg-gradient-to-br ${product.imageColor} p-2 flex flex-col justify-end text-[8px] font-bold text-white/80`}>
              ...
              <div className="text-[10px] text-slate-900 dark:text-white truncate font-extrabold leading-tight">{product.name}</div>
            </div>
          </div>

          {/* Badge */}
          <span className="absolute top-3 left-3 bg-white/90 dark:bg-slate-900/90 text-brand-400 border border-slate-200 dark:border-slate-800 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
            {product.brand}
          </span>
        ...
        <div className="mt-4 space-y-2">
          ...
          {/* Specs overview list */}
          <div className="grid grid-cols-2 gap-1.5 text-[10px] text-slate-600 dark:text-slate-400 border-t border-b border-slate-200 dark:border-slate-800/60 py-2.5">
            <div>
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        <label className="flex items-center gap-2 cursor-pointer py-1 px-2.5 rounded-lg hover:bg-slate-100 dark:bg-slate-850 transition-colors">
          <input 
            type="checkbox"
            checked={isComparing}
            onChange={onToggleCompare}
            className="rounded border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 text-brand-500 focus:ring-brand-500 focus:ring-offset-slate-900 w-4 h-4 cursor-pointer"
          />
          <span className={`text-xs font-semibold select-none ${isComparing ? "text-brand-400 font-bold" : "text-slate-600 dark:text-slate-400 hover:text-slate-200"}`}>
