  ...
  return (
    <>
      <div className="p-6 sm:p-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed">
        <p style={{fontSize: "var(--font-size-p-default, var(--font-size-p-default))"}} className="text-lg font-medium text-slate-900 dark:text-slate-100 mb-6">
          {blog.excerpt}
        </p>

        {blog.content ? (
          <div className="prose prose-sm sm:prose-base dark:prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(blog.content) }} />
        ) : (
          <>
            <p>
