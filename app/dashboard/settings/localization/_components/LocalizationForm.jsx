...
export default function LocalizationForm({ initialSettings }) {
  ...
  const handleSave = () => {
    startTransition(async () => {
      const res = await updateSettings({ ['localization']: settings['localization'] });
      if (res.success) {
        setSuccess(true);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="space-y-6 max-w-2xl">
        <div>
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Default Language (e.g. en)</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Timezone (e.g. UTC)</label>
          <input
            type="text"
