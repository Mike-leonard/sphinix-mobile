...
export default function CommentsForm({ initialSettings }) {
  ...
  const handleSave = () => {
    startTransition(async () => {
      const res = await updateSettings({ ['comments']: settings['comments'] });
      if (res.success) {
        setSuccess(true);
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="space-y-6 max-w-2xl">
        <div className="flex items-center gap-3">
          <input
            type="checkbox"
            checked={settings['comments'].enableComments || false}
            onChange={(e) => handleChange('enableComments', e.target.checked)}
            className="w-5 h-5 rounded border-slate-300 text-brand-600 focus:ring-brand-500"
          />
          <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Enable Comments on Posts</label>
        </div>
        <div className="flex items-center gap-3">
          <input
            type="checkbox"
            checked={settings['comments'].requireApproval || false}
            onChange={(e) => handleChange('requireApproval', e.target.checked)}
            className="w-5 h-5 rounded border-slate-300 text-brand-600 focus:ring-brand-500"
          />
          <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Require Manual Approval</label>
        </div>
      </div>
